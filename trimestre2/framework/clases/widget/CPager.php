	...
	public function dibujaApertura(): string
	{
		...
		if ($paginas > 0) {
			if ($pagActual > 1) {
				...
				$cadena .= " " . CHTML::link(CHTML::dibujaEtiqueta("i", ['class' => "fa-solid fa-angles-left"]) . CHTML::dibujaEtiquetaCierre("i"), $ruta);
				//pagina anterior
				$ruta = $this->_opciones["URL"] . "&pag=" . ($pagActual - 1);
				$cadena .= " " . CHTML::link(CHTML::dibujaEtiqueta("i", ['class' => "fas fa-long-arrow-left"]) . CHTML::dibujaEtiquetaCierre("i"), $ruta);
			}
			//paginas intermedias
			...
			if ($pagActual < $paginas) { //pagina siguiente
				$ruta = $this->_opciones["URL"] . "&pag=" . ($pagActual + 1);
				$cadena .= " " . CHTML::link(CHTML::dibujaEtiqueta("i", ['class' => "fa-solid fa-angles-right"]) . CHTML::dibujaEtiquetaCierre("i"), $ruta);

				//pagina ultima
				$ruta = $this->_opciones["URL"] . "&pag=" . ($paginas);
				$cadena .= " " . CHTML::link(CHTML::dibujaEtiqueta("i", ['class' => "fas fa-long-arrow-right"]) . CHTML::dibujaEtiquetaCierre("i"), $ruta);
			}
		};
		$cadena .= "</div>";
		echo $cadena;
