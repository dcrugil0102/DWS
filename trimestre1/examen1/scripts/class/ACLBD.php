...
class ACLBD extends ACLBase
{
    ...
    function getPermisoRole(int $codRole, int $numero): bool
    {
        ...
        $consulta = "SELECT $permiso from acl_roles " .
            "      where cod_acl_role = $codRole";
    ...
    function getPermisos(int $codUsuario): array|false
    {
        ...
        $consulta = "SELECT r.* from acl_usuarios u " .
            "      inner join acl_roles r on u.cod_acl_role = r.cod_acl_role " .
     ...
     *
     * @param integer $codUsuario Código del usuario a verificar
     * @return boolean Devuelve true si existe, false si no
     */
    function existeCodigoUsuario(int $codUsuario): bool
    {
        if (!$this->_hayConeccion)
            return false;
        ...
        // Consulta rápida para comprobar existencia
        $resul = $this->_sqli->query("SELECT 1 FROM acl_usuarios WHERE cod_acl_usuario = $codUsuario LIMIT 1")->fetch_assoc();

        return !is_null($resul);
    }
}
